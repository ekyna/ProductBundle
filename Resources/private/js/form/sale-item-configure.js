define(['require', 'jquery', 'ekyna-product/templates', 'ekyna-polyfill'], function (require, $, Templates) {
    ...
    function resolveRulesAction(rules, slots) {
        ...
        $.each(rules, function(type, conditions) {
            ...
            $.each(conditions, function (i, cond) {
                ...
                var filter = '[data-index="' + (null === cond.c ? 'null': cond.c) + '"]';
                if (slots[cond.s].$radio.filter(filter).is(':checked')) {
                    if (!all) {
                        fulfill = true;
                        return false;
                    }
    ...
    $.extend(BundleSlot.prototype, {
        ...
        updateState: function() {
            ...
            if (0 < this.$radio.filter(':not([value=""]):not(:disabled)').length) {
    ...
    $.extend(SaleItem.prototype, {
        ...
        resolveAvailability: function() {
            var baseResult = new AvailabilityResult(0);
